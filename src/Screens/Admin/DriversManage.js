...
const DriversManage = () => {
  ...
  useEffect(() => {
    ...
    const list = [];
    firestore().collection('Drivers').where('status', '==', true).get().then((snapShot) => {
            ...
            data.map((val) => {
              return (
                <View key={val.email}>
                  ...
                  onPress={()=> navigation.navigate('Driver Manage Card',
                  {userName: val.userName, email: val.email, cnic: val.cnic, license: val.license})}>
                    <Image source={require('../../../assets/driver.png')} style={{ width: 90, height: 90, borderRadius: 90 / 2, marginLeft: '35%', marginTop: 10}}/>
